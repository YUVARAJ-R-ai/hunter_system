...
## Phase 7 — Time Management System

### 7.1 The Problem
You over-schedule "My Day" in a motivation burst → get overwhelmed → complete nothing.

### 7.2 My Recommendation: Daily Load Meter + Focus Timer

**Approach A — Daily Capacity Guard (addresses over-scheduling)**
Each quest gets an optional `estimated_minutes` field. "My Day" shows a live load bar:
- Green  (0–240 min / 4 hrs): Healthy — achievable day
- Yellow (240–360 min / 6 hrs): Ambitious — warning shown
- Red    (360+ min): Overloaded — "Your plate is full. The System recommends moving X tasks to tomorrow."

The key insight: 4 focused hours of work is a realistic daily limit. Everything else is wishful thinking.

**Approach B — Pomodoro Focus Timer (tracks actual time per task)**
- Click "Focus" on any quest → starts a 25-min Pomodoro timer
- Timer runs as a floating widget, shows elapsed time on the quest card
- After 25 min: 5-min break prompt, then next round
- Session ends → actual_minutes logged to the quest
- After 4 rounds: forced long break (15 min) with an anime quote
- Small Focus XP bonus: +5 XP per completed Pomodoro round

**Approach C — Morning Planning Ritual (my top recommendation)**
On first login each day, a "Daily Planning" modal appears:
1. "How many hours do you have today?" (slider: 1–8 hrs)
2. App auto-selects a realistic set of tasks from your lists based on estimates + priority
3. You confirm or swap tasks
4. Adds selected tasks to "My Day"

This prevents the motivation-burst over-scheduling problem entirely.

### 7.3 DB Changes

```sql
ALTER TABLE quests ADD COLUMN estimated_minutes INTEGER;
ALTER TABLE quests ADD COLUMN actual_minutes INTEGER NOT NULL DEFAULT 0;
...
CREATE TABLE focus_sessions (
  ...
  duration_minutes INTEGER,
  session_type VARCHAR(20) NOT NULL DEFAULT 'pomodoro', -- pomodoro | freeform
  completed BOOLEAN NOT NULL DEFAULT FALSE
);
```

### 7.4 UI Components
- `DailyLoadBar` — progress bar in My Day header (green/yellow/red)
- `FocusTimer` — floating Pomodoro widget with quest title, timer, pause/stop
- `MorningPlanModal` — shown on first login of the day
- `TimeEstimatePicker` — on quest create/edit (5, 10, 15, 30, 45, 60, 90, 120 min)
- `TimeReport` — end-of-day: estimated vs actual time per task

---

## Phase 8 — List Organization (Hierarchical — Already Partially Built)

### 8.1 Current Structure (already in DB)

```
ListGroups  → like a "Subject Area" or "Domain"
  └── Lists → like a "Course" or "Project"
        └── Quests → individual tasks
```

Your exact use case maps perfectly:
```
Group: "Academics"
  ├── List: "Computer Networks"
  ├── List: "Theory of Computation"
  ├── List: "DBMS"
...
**This structure already exists in the backend.** The gap is purely UI/UX — groups need to feel like real containers, not just labels.

### 8.2 UI Improvements Needed

- Groups shown as colored section headers in sidebar (not just text labels)
- Group header: icon + color + name + task count badge
- Clicking a Group → shows all tasks across all its lists (like a "folder view")
- List context always shown in the quest card (e.g., "Academics › Computer Networks")
- Quick-add bar auto-assigns to current list context

### 8.3 DB Changes

```sql
ALTER TABLE list_groups ADD COLUMN icon VARCHAR(50) DEFAULT 'folder';
ALTER TABLE list_groups ADD COLUMN color VARCHAR(20) DEFAULT '#1e90ff';
ALTER TABLE list_groups ADD COLUMN description TEXT;
```

### 8.4 Default Groups to Seed on Registration

New users get these groups pre-created (they can rename/delete):
- 📚 Academics
- 💪 Fitness
- 💼 Work
- 🛠️ Skills
...
## Phase 9 — Daily Login & Retention System

### 9.1 The Problem
You lose motivation → stop logging in → app becomes useless.

### 9.2 Solutions

**Login Streak (like Duolingo's streak):**
- First login each day → Login Streak++, Login Reward awarded
- Missing a day → streak resets to 0
- Streak shown prominently on profile and sidebar footer
- "Streak at risk" push notification if no login by 8PM
...
Day 2:   +30 XP
Day 3:   +50 XP + 10 Gold
Day 7:   +100 XP + 25 Gold  (7-day milestone)
Day 14:  +150 XP + 50 Gold
Day 30:  +300 XP + 100 Gold + title badge "Consistent Hunter"
```

**System Message on Login (Solo Leveling style):**
Inspired by the "You have a new quest" notification from Solo Leveling.
On each login, the app shows a dramatic "System Message" overlay:
- "Day 7 login detected. The System rewards your discipline."
- "WARNING: You have 3 overdue quests. Report to the dungeon."
- "A new day begins. 4 quests await completion."

**Daily Challenge (auto-generated):**
Each day at midnight, 3 daily challenges are generated from the user's own quests and habits:
- "Complete 2 tasks from Computer Networks"
- "Hit the gym today"
- "Check in on your Sleep habit"
Completing all 3 → +100 bonus XP

**"Don't Break the Chain" Visual:**
- GitHub-style activity calendar on the profile page
- Shows login days, quest completions, workout days
- Visual gaps are motivating to fill
...
CREATE TABLE daily_challenges (
  ...
  completed BOOLEAN NOT NULL DEFAULT FALSE,
  xp_reward INTEGER NOT NULL DEFAULT 33,
  created_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
);
```
...
```
POST  /api/auth/daily-login     ← call on every login, awards streak XP
GET   /api/daily-challenges     ← today's 3 challenges
POST  /api/daily-challenges/:id/complete
GET   /api/activity-calendar    ← login + completion history for calendar
```

### 9.5 Notifications (PWA)

- Register service worker for web push notifications
- "Streak reminder" at 8PM if no login today
- "Boss HP is regenerating — login to deal damage" (gamified nudge)
- "Your habit streak is at risk!" after 22 hours of no check-in

---

## Phase 10 — Mobile-First Responsive Design

### 10.1 Current Mobile State

The sidebar is `hidden md:flex` — completely invisible on phone. 
The quest grid is `grid-cols-1 sm:grid-cols-2` — workable but not optimized.
All modals are desktop-centered, not full-screen on mobile.

### 10.2 Mobile Navigation (Bottom Tab Bar)

Replace the hidden sidebar with a bottom tab bar on mobile:

```
[ 🌤 My Day ] [ ⚡ Quests ] [ 👤 Profile ] [ 🏋️ Gym ] [ 💰 Finance ]
```

- Active tab has glow indicator
- Swipe left/right between tabs
- Lists sidebar accessible via hamburger → slide-in drawer

### 10.3 Mobile-Specific UX Rules

| Element | Desktop | Mobile |
|---|---|---|
| Sidebar | Fixed left panel | Hidden — bottom tab bar |
| Modals | Centered overlay | Full-screen bottom sheet |
| Quest cards | 2-column grid | Single column, full width |
| Buttons | Normal size | Minimum 44px tap target |
| Quick-add | Bottom bar | FAB → expands to full input |
| Focus timer | Floating widget | Sticky header bar |
| Muscle heatmap | Side by side (front+back) | Swipeable front/back |
| Charts | Full width | Horizontally scrollable |

### 10.4 Swipe Gestures (Mobile)

- **Swipe right on quest card** → Mark complete (green flash)
- **Swipe left on quest card** → Delete/fail options appear
- **Swipe right on habit** → Check in
- **Long press on quest** → Context menu (edit, move to list, mark important)

### 10.5 PWA (Progressive Web App) Setup

Make the web app installable on Android home screen:
...
- App opens in standalone mode (no browser chrome) — feels native
...
- Render `BottomTabBar` when `isMobile === true`
- All modals use `motion.div` sliding up from bottom on mobile
- CSS: replace fixed-width cards with `w-full` on mobile breakpoints

---

## Updated Delivery Order (All Phases)

| Priority | Feature | Effort |
|----------|---------|--------|
| 🔥 Critical | Bug fixes (dupe logout, boss prompt, reward reset) | 0.5 day |
| 🔥 Critical | Habit check-in XP rewards | 0.5 day |
| 🔥 Critical | Sub-task HP award + momentum bar | 1 day |
| 🔥 Critical | Flutter Android SDK setup (NixOS) | 2–4 hrs |
| 🔥 Critical | Mobile-first: Bottom tab bar + full-screen modals | 1 day |
| 🔥 Critical | PWA manifest + service worker (installable on phone) | 0.5 day |
| ⚡ High | Daily login streak + login reward system | 1 day |
| ⚡ High | System Message login overlay (Solo Leveling style) | 0.5 day |
| ⚡ High | List Groups: icons, colors, default seed on register | 0.5 day |
| ⚡ High | Anime quotes + themed visual system | 1 day |
| ⚡ High | Boss UI overhaul + linked quest damage | 1 day |
| ⚡ High | Daily Load Meter in My Day | 1 day |
| ⚡ High | Finance / Wallet module — full stack | 3–4 days |
| 📌 Medium | Focus Timer (Pomodoro) + actual_minutes tracking | 1.5 days |
| 📌 Medium | Morning Planning Modal | 1 day |
| 📌 Medium | Daily Challenges (auto-generated 3/day) | 1 day |
| 📌 Medium | Activity Calendar (login + quest history) | 1 day |
| 📌 Medium | Swipe gestures on mobile | 1 day |
| 📌 Medium | Gym Logger: DB + API + 200+ exercise seed | 2–3 days |
| 📌 Medium | Gym Logger: workout session UI | 2 days |
| 📌 Medium | Muscle heatmap + analytics | 1–2 days |
| 📌 Medium | Quick-add bar + quest edit + list rename | 1 day |
| 🌀 Later | Push notifications (PWA) | 1 day |
| 🌀 Later | Mobile Flutter: Finance + Gym | 2–3 days |
| 🌀 Later | Drag-and-drop + global search | 1 day |
